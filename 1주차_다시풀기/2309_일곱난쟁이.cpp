...
// 난쟁이의 키의 합이 100이 됨을 기억해 냈다. 아홉 난쟁이의 키가 주어졌을 때,
// 백설공주를 도와 일곱 난쟁이를 찾는 프로그램을 작성하시오.

// 입력

// 아홉 개의 줄에 걸쳐 난쟁이들의 키가 주어진다. 주어지는 키는 100을 넘지 않는
// 자연수이며, 아홉 난쟁이의 키는 모두 다르며, 가능한 정답이 여러 가지인
// 경우에는 아무거나 출력한다.
...
// 9개 중 7개를 뽑아서 100이 되는 지를 확인한다.
// 전체 합을 구해놓고 2개를 뽑아서 빼서 100이 되는지 확인한다.
// 조합 -> 완탐
...
void print(int del1, int del2) {
    sort(a, a + n);
    for (int i = 0; i < n; i++) {
        if (a[i] == del1 || a[i] == del2) continue;
        cout << a[i] << '\n';
    }
}
